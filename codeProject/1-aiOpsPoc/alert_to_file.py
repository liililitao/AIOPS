...
# -*- coding: utf-8 -*-
"""
Splunk Alert Action Script - 告警信息本地持久化
==============================================
用途: 替代邮件告警，将 Splunk 告警上下文和搜索结果写入本地文件
部署: 复制到 /data/splunk/bin/scripts/alert_to_file.py
      chown splunk:splunk /data/splunk/bin/scripts/alert_to_file.py
      chmod 755 /data/splunk/bin/scripts/alert_to_file.py

Splunk 告警动作配置中填写脚本名: alert_to_file.py
"""
...
OUTPUT_DIR = "/data/splunk/var/log/splunk/waf_alerts"
...
def append_index(content, json_filename):
    """
    追加一行汇总到索引文件，格式:
    [2026-07-09 09:59:00] test-waf | 命中25条 | id=xxx hostname=xxx | 详见 xxx.json
    """
    ...
    if results:
        ...
        # 如果 requestUri 是多值的 values() 输出，截短显示
        if len(uris) > 80:
            uris = uris[:77] + "..."
        summary = f"id={rid} hostname={host} uri={uris}"

    line = f"[{now}] {alert_name} | 命中{count}条 | {summary} | 详见 {json_filename}\n"
...
def main():
    print(f"[*] alert_to_file.py 启动, PID={os.getpid()}")
